## 更换美区账号{docsify-ignore}

* 打开`App Store`-->点`右上角头像`-->`下拉到底部`-->`退出登录`，再登录`美区账号`[<a href="javascript:getid(0);">点击查看账号</a>]

<div style="color:Blue" id="jsonTip"></div>

* 登陆时有安全提示，点`其他选项`-->`不升级`，否则会弹出安全问题！[登录后下载 [Shadowrocket](https://apps.apple.com/us/app/shadowrocket/id932747118)]

![apple](media/apple/id.jpg ':size=360')

!> 下载完App后，请退出`美区账号`，以避免各种`潜在的安全问题！`

!> `不要`在手机的`设置`里登录`美区账号`。否则，您的设备可能会`变成砖头！`

!> 如提示`帐户已在 App Store 被禁用`，请删掉手机里已安装的`Shadowrocket`

## Shadowrocket (小火箭)

* 支持 iOS 12.x - 15.x 系统

* [更换美区账号](ios?id=更换美区账号) 后在`App Store`搜索`Shadowrocket`[`下载完后，请退出美区账号`]

### 快速订阅

**快速订阅一次添加所有节点**

一、登入您购买`SS节点`网站，`订阅链接`-->`Shadowrocket`-->`导入App`

![android](media/apple/sub_1.jpg ':size=360')

二、打开`小火箭`-->`选择节点`-->`连接/断开`[`如提示VPN权限，请允许`]

![android](media/apple/sub_2.jpg ':size=360')

### 逐个添加

**此法每次只能添加一个节点**

* 登入您购买`SS节点`的网站，复制`二维码链接`[每个节点都对应一个`二维码链接`]

![Shadowrocket](media/apple/sr_1.jpg ':size=360')

* 打开小火箭-->自动检测到`SS链接`-->点击`添加`[`重复操作`添加其他节点到App]

![Shadowrocket](media/apple/sr_2.jpg ':size=360')

* `另种方法`添加节点：点击`SS节点`的二维码 [点两次]-->检测到`SS链接`-->`打开`

![Shadowrocket](media/apple/sr_3.jpg ':size=360')

* `选择节点`-->`连接/断开`首次使用会提示添加代理-->点击`Allow`允许VPN连接！

![Shadowrocket](media/apple/sr_4.jpg ':size=360')

!> 常见问题

`[1]` 确认设置没问题，但还是不能用！

切换 4G/wifi 网络进行尝试，不行再重启`手机/路由器`试一试！

`[2]` 为什么没看到`Potatso Lite`的教程？

因为`Potatso Lite`不支持 <a href="https://shadowsocks.org/en/wiki/SIP002-URI-Scheme.html" target="_blank">SIP002</a>，`可能需要`手动设置插件。[点击这里](potatso-lite) 查看教程！